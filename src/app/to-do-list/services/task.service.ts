...
export class TaskService {
  ...
  // Efeito para persistir as tarefas no localStorage sempre que a lista muda
  constructor() {
    effect(() => {
  ...
  // Método para carregar as tarefas do localStorage ou usar dados padrão
  loadTasks(): void {
    const defaultTasks: Task[] = [
      { id: 1, name: 'Falar com o Filipe Zuzarte', creationDate: new Date('2023-10-10T20:00:00'), finished: false },
      { id: 2, name: 'Fazer deploy do projeto', creationDate: new Date('2023-10-10T19:00:00'), finished: false },
      { id: 3, name: 'Fazer a folha de horas', creationDate: new Date('2023-10-10T18:00:00'), finished: false }
    ];

    const savedTasks = localStorage.getItem('tarefas');
    if (savedTasks) {
      this._tasks.set(JSON.parse(savedTasks).map((t: any) => ({
        ...t,
